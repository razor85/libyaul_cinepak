...
typedef struct {
  char
    loopType; //[0,1,2,3] No loop, normal loop, reverse loop, alternating loop
  ...
  unsigned short
    playsize; // The # of samples to play before the sound shall loop.
  ...
  unsigned short
    decompression_size;     // Size of the buffer used for an ADX sound effect.
  ...
  char icsr_target; // Which explicit ICSR is this to land in? Can be controlled
                    // by SH2 or by driver.
} _PCM_CTRL;        // Driver Local Command Struct

typedef struct {
  volatile unsigned int
    adx_stream_length;              // Length of the ADX stream (in ADX frames)
  ...
  volatile unsigned char
    cdda_left_channel_vol_pan; // Redbook left channel volume & pan.
  volatile unsigned char
    cdda_right_channel_vol_pan; // Redbook right channel volume & pan.
} sysComPara;
