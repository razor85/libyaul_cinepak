...
#  define DEBUG_REQUIRE(A)                                                                                      \
    do {                                                                                                        \
      if (!(A)) {                                                                                               \
        Memory::libAssert(__FILE__, __LINE__, "Error (" #A " is false) at " __FILE__ ":" __TOSTRING(__LINE__)); \
      }                                                                                                         \
    } while (false)
...
public:
  static void initialize() {
    tlsfHandle = user_tlsf_create_with_pool(reinterpret_cast<void *>(LWRAM(0)), static_cast<size_t>(1024 * 1024));
  }

  static void runTests() { DEBUG_REQUIRE_EQ(user_tlsf_check(tlsfHandle), 0); }

  static void *alloc(uint32_t size) { return user_tlsf_malloc(tlsfHandle, size); }
  ...
  static void *memalign(uint32_t align, uint32_t size) { return user_tlsf_memalign(tlsfHandle, align, size); }

  static void *realloc(void *ptr, uint32_t size) { return user_tlsf_realloc(tlsfHandle, ptr, size); }

  static void free(void *ptr) { return user_tlsf_free(tlsfHandle, ptr); }
  ...
  static uint32_t getFreeBytes(bool calculate = true) { return getTotalBytes() - getUsedBytes(calculate); }

  static uint32_t getTotalBytes() { return 1024 * 1024; }
...
public:
  ...
  static void initialize() { tlsfHandle = user_tlsf_create_with_pool(data, maxSize); }
  ...
  static void freeAll() { initialize(); }
  ...
  static uint32_t getTotalBytes() { return maxSize; }
...
public:
  static void initialize() {
    ...
    const uint32_t cartSize{cartId == DRAM_CART_ID_INVALID ? 0UL : cartId == DRAM_CART_ID_1MIB ? 1UL : 4UL};

    if (cartSize > 0) {
      totalMemory = cartSize * 1024 * 1024;
      tlsfHandle = user_tlsf_create_with_pool(dram_cart_area_get(), totalMemory);
    } else {
      totalMemory = 0;
    }
  }
  ...
  static uint32_t getTotalBytes() { return totalMemory; }
...
public:
  ...
  void pointToPreallocatedArea(Array<T> &inArray, uint32_t inNumItems, bool construct = false) {
  ...
  void pointToPreallocatedArea(T *ptr, uint32_t inNumItems, uint32_t inCapacity, bool construct = false) {
    DEBUG_REQUIRE_EQ(dataPtr, nullptr);
  ...
  void allocateHWRAM(uint32_t numItems, bool fillCapacity = false) {
    ...
    if (numItems > 0) {
      T *highAllocatedNewData{Memory::High::memalign<T>(alignof(T), totalCapacity * sizeof(T))};
  ...
  void allocateDRAM(uint32_t numItems, bool fillCapacity = false) {
    ...
    if (numItems > 0) {
      T *highAllocatedNewData{Memory::DRAM::memalign<T>(alignof(T), totalCapacity * sizeof(T))};
  ...
  Span span(uint32_t inBegin) { return Span(*this, inBegin, count - inBegin); }

  Span span(uint32_t inBegin, uint32_t inCount) { return Span(*this, inBegin, inCount); }
